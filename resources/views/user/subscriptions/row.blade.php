...
<x-livewire-tables::table.cell>
    <div class="flex items-center">
        <div class="text-sm font-medium text-gray-900">
            @if($row->payment_status == 0)
                <a href="{{ route('subscriptions-show', $row->id) }}" class="inline-flex items-center px-10 py-2 bg-indigo-800 rounded-md font-semibold text-xs text-white text-center">{{ __('Stripe') }}</a>
            @else
                {{ __('') }}
            @endif
        </div>
