    ...
    <div class="py-10">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden sm:rounded-lg">
                <x-card-title title="{{ __('Edit role') }}"></x-card-title>
                            ...
                            <div class="md:grid md:grid-cols-3 md:gap-6">
                                ...
                                <div class="mt-5 md:mt-0 md:col-span-2">
                                    <div class="grid grid-cols-1 mt-5">
                                        ...
                                        <x-input name="name" value="{{ $role->name }}"></x-input>
                                    </div>
                                    <div class="grid grid-cols-1 mt-5">
                                        <x-label for="permission" value="{{ __('Permissions') }}"></x-label>
                                        @foreach($permissions as $permission)
                                            <x-checkbox name="checked[]" checked="{{ $role->hasPermissionTo($permission['name']) }}" value="{{ $permission->id }}">
